<template>
  <body>
    <div class="container">
      <div class="row">
        <div class="col-sm-12">
          <h2>Magu Flower Collection</h2>
          <p></p>
        </div>
        <div class="col-sm-12">
          <div class="portfolio grid-4">
            <figure>
              <img src="images/Category1/ACDC.png" alt="">
              <figcaption>
                <h2>ACDC</h2>
                <p>CBD-Heavy Strain</p><router-link to="/strains/13"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category2/Blue-Blood-bud.png" alt="">
              <figcaption>
                <h2>Blue Blood</h2>
                <p>THC-Heavy Strain</p><router-link to="/strains/21"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category1/bluejayway.jpg" alt="">
              <figcaption>
                <h2>Blue Jay Way</h2>
                <p>CBD-Heavy Strain</p><router-link to="/strains/16"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category1/Cannatonic.jpg" alt="">
              <figcaption>
                <h2>Cannatonic</h2>
                <p>CBD-Heavy Strain</p><router-link to="/strains/15"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category3/CBD-Shark.jpg" alt="">
              <figcaption>
                <h2>CBD Shark</h2>
                <p>1:1 Ratio Strain</p><router-link to="/strains/20"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category1/Doctor-Doctor.jpg" alt="">
              <figcaption>
                <h2>Doctor Doctor</h2>
                <p>CBD-Heavy Strain</p><router-link to="/strains/3"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category1/GI001-Bud.jpg" alt="">
              <figcaption>
                <h2>GI001</h2>
                <p>CBD-Heavy Strain</p><router-link to="/strains/14"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category2/Girl-Scout-Cookies.jpg" alt="">
              <figcaption>
                <h2>Girl Scout Cookies</h2>
                <p>THC-Heavy Strain</p><router-link to="/strains/5"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category3/harlequin.jpg" alt="">
              <figcaption>
                <h2>Harlequin</h2>
                <p>1:1 Ratio Strain</p><router-link to="/strains/10"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category2/Hindu-Kush.jpg" alt="">
              <figcaption>
                <h2>Hindu Kush</h2>
                <p>THC-Heavy Strain</p><router-link to="/strains/7"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category3/In-The-Pines.jpg" alt="">
              <figcaption>
                <h2>In The Pines</h2>
                <p>1:1 Ratio Strain</p><router-link to="/strains/9"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category3/Jamaican-Lion.jpeg" alt="">
              <figcaption>
                <h2>Jamaican Lion</h2>
                <p>1:1 Ratio Strain</p><router-link to="/strains/11"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category2/Northern-Lights.jpg" alt="">
              <figcaption>
                <h2>Northern Lights</h2>
                <p>THC-Heavy Strain</p><router-link to="/strains/8"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category1/Oracle.jpg" alt="">
              <figcaption>
                <h2>Oracle</h2>
                <p>CBD-Heavy Strain</p><router-link to="/strains/18"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category2/Pineapple-Sage.jpeg" alt="">
              <figcaption>
                <h2>Pineapple Sage</h2>
                <p>THC-Heavy Strain</p><router-link to="/strains/23"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category1/Remedy.jpg" alt="">
              <figcaption>
                <h2>Remedy</h2>
                <p>CBD-Heavy Strain</p><router-link to="/strains/2"></router-link>
              </figcaption>
              </figure>
              <figure>
                <img src="images/Category2/Skywalker-OG.jpg" alt="">
                <figcaption>
                  <h2>Skywalker OG</h2>
                  <p>THC-Heavy Strain</p><router-link to="/strains/6"></router-link>
                </figcaption>
            </figure>
            <figure>
              <img src="images/Category3/Sour-Tsunami.jpg" alt="">
              <figcaption>
                <h2>Sour Tsunami</h2>
                <p>1:1 Ratio Strain</p><router-link to="/strains/17"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category3/Star-Tonic.jpg" alt="">
              <figcaption>
                <h2>Star Tonic</h2>
                <p>1:1 Ratio Strain</p><router-link to="/strains/19"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category1/Suzy-Q.jpg" alt="">
              <figcaption>
                <h2>Suzy Q</h2>
                <p>CBD-Heavy Strain</p><router-link to="/strains/4"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category3/The-Ox-Bud.jpg" alt="">
              <figcaption>
                <h2>The Ox</h2>
                <p>1:1 Ratio Strain</p><router-link to="/strains/22"></router-link>
              </figcaption>
            </figure>

            <figure>
              <img src="images/Category1/Valentine-X.jpg" alt="">
              <figcaption>
                <h2>Valentine X</h2>
                <p>CBD-Heavy Strain</p><router-link to="/strains/1"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category3/RX.jpg" alt="">
              <figcaption>
                <h2>RX</h2>
                <p>1:1 Ratio Strain</p><router-link to="/strains/12"></router-link>
              </figcaption>
            </figure>
            <figure>
              <img src="images/Category2/granddaddypurple.jpg">
              <figcaption>
                <h2>Granddaddy Purple</h2>
                <p>THC-Heavy Strain</p>
              </figcaption>
            </figure>
            <div class="left">
              
              <img src="images/merakilabbe.jpg"><br>
                
              <router-link to="/"><button type="button" class="btn btn-custom btn-sm">Home</button></router-link>

            </div>
          </div>
        </div>
      </div>
    </div>
  </body>
</template>

<style>
  .left {
    position: center;
  }
</style>
